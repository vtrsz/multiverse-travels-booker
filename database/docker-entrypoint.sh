...
docker_process_sql_in_test_db() {
	local query_runner=( psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --no-password --no-psqlrc )

	if [ -n "$POSTGRES_DB_TEST" ]; then
		query_runner+=( --dbname "$POSTGRES_DB_TEST" )
	fi
...
# create initial database
# uses environment variables for input: POSTGRES_DB_TEST
docker_setup_test_db() {
	local dbAlreadyExists
	dbAlreadyExists="$(
		POSTGRES_DB_TEST= docker_process_sql_in_test_db --dbname postgres --set db="$POSTGRES_DB_TEST" --tuples-only <<-'EOSQL'
			SELECT 1 FROM pg_database WHERE datname = :'db' ;
		EOSQL
	)"
	if [ -z "$dbAlreadyExists" ]; then
		POSTGRES_DB_TEST= docker_process_sql_in_test_db --dbname postgres --set db="$POSTGRES_DB_TEST" <<-'EOSQL'
			CREATE DATABASE :"db" ;
		EOSQL
		printf '\n'
	fi
